...
Pour gagner de l'argent, des visiteurs (non-controlé par les joueurs\index{joueur}) doivent entrer dans un de leur établissement, y rester une durée déterminée, leur donner l'argent adapté à l'établissement où il se trouve, et disparaitra ensuite de la partie.
...
\begin{itemize}
 ...
 \item \textbf{Serveur:}\index{serveur} Ordinateur dédié à l'administration d'un réseau informatique\index{réseau informatique}. Il gère l'accès aux ressources et aux périphériques et les connexions des différents utilisateurs.
...
Une fois le jeu\index{jeu} démarrer, l'utilisateur peut se connecté insérant un nom d'utilisateur existant et le mot de passe associé
...
connecté. Un utilisateur non-enregistré peut également créer un nouveau pseudo\index{pseudo} unique
...
[Exception: aucun pseudo\index{pseudo} et/ou mot de passe entré]. Ceci ajoutera sur le \textit{server} un nouvel utilisateur avec comme
pseudo\index{pseudo} et mot de passe associés ceux entrés dans les zones appropriées, et connectera l'utilisateur automatiquement.
...
\begin{itemize}
 ...
 demande à l'utilisateur de réentrer son pseudo\index{pseudo} et son mot de passe.
 \item \textit{Pseudo\index{pseudo} déja utilisé:} Après la pression du bouton \og \textit{Create} \fg,
 sur la même vue, une remarque apparait, en une couleur fortement
 perceptible, et indique à l'utilisateur que le pseudo\index{pseudo} demandé est déjà utilisé. La fenêtre de création reste ouverte
 et demande à l'utilisateur de réentrer un nouveau pseudo\index{pseudo}.
...
\end{itemize}
\paragraph{Cas général}
Un joueur\index{joueur} peut se déconnecté à n'importe quel moment. Si il est en partie, il la quitte automatiquement. L'utilisateur
...
du menu principal.
Il est ensuite envoyé dans l'écran de sélection du jeu\index{jeu}, ou il lui est possible de voir toutes les parties lancées sur le
...
Après avoir rejoins une partie, si celle-ci n'est pas commencé le joueur\index{joueur} est placé dans l’accueil où il peut
voir tous les joueurs\index{joueur} en attente - du début de la partie -, et la carte choisie. Il lui
est également possible de chatter avec les autres joueurs\index{joueur}. 
[Exception: Hôte ferme la partie]
Dans le cas où la partie est déjà commencé le joueur\index{joueur} reçoit de l'argent.
\paragraph{Exceptions}
\begin{itemize}
...
\paragraph{Postconditions}
\begin{itemize}
 \item Une partie est ajoutée sur le \textit{serveur\index{serveur}}.
...
Il choisit d'abords une carte\index{carte} et optionnellement d'autres options; la partie est alors créée, mais n'est pas lancée tant qu'il n'y a pas au moins 2 joueurs\index{joueur} qui participent.
Quand il y a assez de joueur\index{joueur} - minimum 2 et maximum 8 - le bouton \og \textit{Start game} \fg s'active. Quand il est cliqué, la partie démarre.
\paragraph{Exceptions} Néant.
...
\end{itemize}
\paragraph{Cas général}
Dès qu'un bâtiment n'as pas de prioritaire, le \textit{catalogue}\index{catalogue} le met en vente libre. Les bâtiments créés en début de partie seront mit dans le \textit{catalogue}\index{catalogue}, ainsi que les bâtiments d'un joueur\index{joueur} qui vient de perdre qui sont mit en vente libre.
\paragraph{Exceptions} Néant.
...
\paragraph{Préconditions}
\begin{itemize}
	\item Le joueur\index{joueur} est dans une partie, sur l'écran de jeu\index{jeu}.
\end{itemize}
\paragraph{Postconditions}
\begin{itemize}
...
\end{itemize}
\paragraph{Cas général}
A tout moment de la partie, un joueur\index{joueur} peut tenter d'acheter le bâtiment d'un autre. Il lui doit simplement de faire une offre auprès de ce joueur\index{joueur}, pour ce bâtiment.[Exception : Le joueur\index{joueur} a fait trop d'offre] Il suffit à l'acheteur de sélectionner le bâtiment voulu et de cliquer sur \og \textit{Faire une offre} \fg, une fenêtre s'ouvre pour que le joueur\index{joueur} puisse donner le prix qu'il est prêt a payer et confirme son offre.[Exception : Le joueur\index{joueur} n'a pas assez d'argent] 
\paragraph{Exceptions}
\begin{itemize}
 \item \textit{Le joueur\index{joueur} n'a pas assez d'argent:} Une fenêtre s'ouvre indiquant au joueur\index{joueur} qu'il ne possède pas assez d'argent.
...
Dans le cas d'un crash ou d'arrêt de \textit{serveur}\index{serveur}, il faut faire en sorte que les joueurs\index{joueur} puissent reprendre leur partie. Le \textit{serveur}\index{serveur} fera donc une sauvegarde automatique à chaque debut de journée(de la partie en cours, 7h du matin). 
...
Quand le \textit{serveur}\index{serveur} re-démarre, il vérifie si une partie en cours a été interrompue; si oui, il relance la partie grâce à sa sauvegarde.
...
\begin{itemize}
 \item Le jeu\index{jeu} est multi-joueur\index{multi-joueur}, et doit donc permettre aux différents utilisateurs de communiquer entre eux.
 \item Une partie doit être composé d'au minimum 2 joueurs\index{joueur} et au maximum 8 joueurs\index{joueur}.
 \item Les 8 premiers joueurs\index{joueur} qui rejoignent la partie seront les seules 8 joueurs\index{joueur} capable de la rejoindre.
 ...
 un capital. 
 \item Le capitale de chaque joueur\index{joueur} varie tout au long de la partie en fonction des frais et des gains de chaque bâtiment.
 ...
 \item Si le joueur\index{joueur} est en faillite, la partie se terminera pour ce joueur\index{joueur} et il ne sera plus possible pour lui de
 continuer de jouer dans cette partie, mais il pourra toute fois la regarder.
 \item La partie se finie uniquement si il ne reste plus qu'un joueur\index{joueur} propriétaire.
